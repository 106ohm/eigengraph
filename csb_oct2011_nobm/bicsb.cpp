...
template <class NT, class IT>
BiCsb<NT, IT>::BiCsb (Csc<NT, IT> & csc, int workers):nz(csc.nz),m(csc.m),n(csc.n)
{
...
template <class NT, class IT>
template <typename SR, typename RHS, typename LHS>
void BiCsb<NT, IT>::BTransMult(IT col, IT rowstart, IT rowend, const RHS * __restrict x, LHS * __restrict y, IT ysize) const
{
	IT * blocknnz = new IT[rowend-rowstart];	// nnz per block responsible
	for(IT i=rowstart; i<rowend; ++i)
	{
		blocknnz[i-rowstart] = top[i][col+1] - top[i][col];
	}
	IT sum = std::accumulate(blocknnz, blocknnz + (rowend-rowstart), 0);		// initial value = 0
	delete [] blocknnz;
	...
	if(sum < BREAKEVEN * ysize)		// not enough nonzeros to amortize new array formation
	{
		...
		if(sum != 0)	// avoid function call and loop set up
			SubSpMVTrans<SR>(col, rowstart, rowend, x, y);
	}
	else if(rowend-rowstart == 1)	// only one block remains and it is relatively dense
	{
		...
		{
			IT chi = (rowstart << rowlowbits);
			const RHS * __restrict subx = &x[chi];
			BlockParT<SR>( top[rowstart][col] , top[rowstart][col+1],  subx, y, 0, blcrange, BREAKEVEN * ysize);
		}
		else
		{
			SubSpMVTrans<SR>(col, rowstart, rowend, x, y);
	...
	{
		IT rowmid = (rowend+rowstart)/2;
		cilk_spawn BTransMult<SR>(col, rowstart, rowmid, x, y, ysize);
		if(SYNCHED)
		{
			BTransMult<SR>(col, rowmid, rowend, x, y, ysize);
		}
		else
		{
			LHS * temp = new LHS[ysize]();
			BTransMult<SR>(col, rowmid, rowend, x, temp, ysize);
			cilk_sync;
...
template <class IT>
template <typename SR, typename RHS, typename LHS>
void BiCsb<bool, IT>::BTransMult(IT col, IT rowstart, IT rowend, const RHS * __restrict x, LHS * __restrict y, IT ysize) const
{
	IT * blocknnz = new IT[rowend-rowstart];	// nnz per block responsible
	for(IT i=rowstart; i<rowend; ++i)
	{
...
template <class NT, class IT>
template <typename SR, typename RHS, typename LHS>
void BiCsb<NT, IT>::SubSpMVTrans(IT col, IT rowstart, IT rowend, 
							const RHS * __restrict x, LHS * __restrict suby) const
{
	IT * __restrict r_bot = bot;
	NT * __restrict r_num = num;
	for(IT i= rowstart; i < rowend; ++i)
...
template <class IT>
template <typename SR, typename RHS, typename LHS>
void BiCsb<bool, IT>::SubSpMVTrans(IT col, IT rowstart, IT rowend, 
							const RHS * __restrict x, LHS * __restrict suby) const
{
	IT * __restrict r_bot = bot;
	for(IT i= rowstart; i < rowend; ++i)
